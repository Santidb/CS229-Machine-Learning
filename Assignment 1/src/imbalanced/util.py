...
def plot(x, y, theta, save_path):
    ...
    # Plot dataset
    plt.figure()
    plt.plot(x[y == 1, -2], x[y == 1, -1], 'bx', linewidth=2, label='Class 1', alpha=0.3)
    plt.plot(x[y == 0, -2], x[y == 0, -1], 'go', linewidth=2, label='Class 0', alpha=0.3)

    # Plot decision boundary (found by solving for theta^T x = 0)
    x1 = np.arange(min(x[:, -2])-4, max(x[:, -2])+4, 0.01)
    x2 = -(theta[0] / theta[2] + theta[1] / theta[2] * x1)
    plt.plot(x1, x2, c='red', linewidth=2, alpha=0.7)
